...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		{
			if(USART_RX_STA&0x4000)//判断STA Bit14是否为1，接收到了0x0d,
			{
			...
			{	
				if(Res==0x0d)USART_RX_STA|=0x4000;//标记bit14
				else
				{
					USART_RX_BUF[USART_RX_STA&0X3FFF]=Res ;
					USART_RX_STA++;
					if(USART_RX_STA>(USART_REC_LEN-1))USART_RX_STA=0;//数据太多，接收数据错误,重新开始接收	  
				}		 
			}
		}   		 
  } 
...
#endif	//THIS ENDIF is for :#if EN_USART1_RX
...
void uart6_init(u32 bound6){
  ...
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART6,ENABLE);//使能USART6时钟,和USART1所用时钟是一样的
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7; //GPIOC6与GPIOC7
   ...
   //USART6 初始化设置
	USART_InitStructure.USART_BaudRate = bound6;//波特率设置
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;//字长为8位数据格式
	USART_InitStructure.USART_StopBits = USART_StopBits_1;//一个停止位
	USART_InitStructure.USART_Parity = USART_Parity_No;//无奇偶校验位
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;//无硬件数据流控制
...
void USART6_IRQHandler(void)//串口6中断服务程序
  {      
	...
	{
		res6 =USART_ReceiveData(USART6);//(USART1->DR);	//读取接收到的数据
		if((USART6_RX_STA&0x8000)==0)//接收未完成
		{
			if(USART6_RX_STA&0x4000)//接收到了0x0d
			{
				if(res6!=0x0a)USART6_RX_STA=0;//接收错误,重新开始
